...
export function behaviorRepairTargets(bytes: number): [number, number] {
  ...
  if (normalizedBytes < 500) return [506, 518];
  if (normalizedBytes > 550) return [544, 532];
...
export function behaviorRepairInstruction(bytes: number) {
  ...
  return `현재 ${plan.bytes}바이트이므로 후보 1은 ${targets[0]}바이트, 후보 2는 ${targets[1]}바이트를 각각 목표로 함. ${location} 한글 약 ${plan.syllables}음절(${Math.abs(plan.byteDelta)}바이트 안팎)만 ${action}함. ${editScope}. 문장 전체를 다시 쓰거나 순서를 바꾸지 말고, 각 후보를 직전 본문과 대조한 뒤 UTF-8 바이트를 다시 계산함. 모든 문장은 마침표 직전 글자가 받침 ㅁ인 ‘음/임/함/됨’ 형태인지 확인하고 500~550바이트 범위에 들어오면 더 수정하지 않음`;
}
