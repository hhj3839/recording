...
export function behaviorRepairInstruction(bytes: number) {
  ...
  if (plan.direction === "none") {
    return `현재 ${plan.bytes}바이트로 500~550바이트 기준을 충족하므로 문장 수·순서·길이와 핵심 사실을 유지하고, 다른 검수 오류가 있을 때만 같은 의미의 표현으로 최소 교체함`;
  }
  ...
  return `현재 ${plan.bytes}바이트이므로 ${location} 한글 약 ${plan.syllables}음절(${Math.abs(plan.byteDelta)}바이트 안팎)만 ${action}하여 ${plan.targetBytes}바이트 부근으로 조정. ${editScope}. 문장 전체를 다시 쓰거나 순서를 바꾸지 말고 500~550바이트 범위에 들어오면 더 수정하지 않음`;
}
