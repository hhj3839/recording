...
export function commentPoolQuality(sentences: string[], referenceSentence = "") {
  const normalized = sentences.filter((sentence) => !hasAbilityStatement(sentence) && hasNaturalNominalEnding(repairSafeNominalEnding(sentence))).map(normalizedPoolSentence).filter(Boolean);
