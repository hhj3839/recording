...
import { buildCanonicalCommentSentence, hasNaturalNominalEnding, positiveGrowthCriterion, repairSafeNominalEnding } from "./comment-generation-policy.ts";
...
export function commentPoolQuality(sentences: string[], referenceSentence = "") {
  const normalized = sentences.filter((sentence) => !hasAbilityStatement(sentence) && hasNaturalNominalEnding(stable(sentence))).map(normalizedPoolSentence).filter(Boolean);
...
export function validatePoolCandidate(candidate: string, spec: CommentPoolSpec) {
  ...
  if (hasAbilityStatement(text)) issues.push(ABILITY_STATEMENT_ISSUE);
