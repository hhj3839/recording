  ...
  try {
    ...
    const response = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        ...
        input: [
          { role: "system", content: [{ type: "input_text", text: "너는 대한민국 초등학교 담임교사이며 학기말 학교생활기록부의 행동특성 및 종합의견을 작성하는 전문가이다. 입력된 학교생활 관찰 사실만 활용하여 학생의 학교생활 전반이 종합적으로 이해되게 작성한다. 장점과 노력은 입력에 있을 때만 긍정적이고 균형 있게 드러내며 입력에 없는 사실·변화·성장 가능성을 만들지 않는다. 부정적인 내용은 사실을 바꾸지 않는 범위에서 관찰 가능한 모습으로 순화하고 과장·단정·학생 간 비교를 피한다. 학생 이름과 성별을 쓰지 않는다. 성적·등수·수상 실적·대회·사교육·공인시험·특정 기관명·가정환경·부모 직업·사회경제적 배경·신체조건·학교폭력·징계·질병을 포함하지 않는다. variation을 따르되 기존 학생 문장과 표현을 반복하지 않는다. 문장 수를 고정하지 않고 줄바꿈 없는 자연스러운 한 문단으로 작성한다. UTF-8 500~600바이트 범위로 작성하고 모든 문장은 ‘음/임/함/됨’ 형태의 자연스러운 명사형으로 끝낸다. 출력 전 분량·종결·반복·입력 사실 일치·금지 내용·맞춤법을 검수하며 제목이나 설명 없이 본문만 출력한다." }] },
          { role: "user", content: [{ type: "input_text", text: mode === "length" && currentBehavior
            ...
            : `다음 학생 특성을 바탕으로 표현과 문장 구조가 다른 행동특성을 작성해 줘. 문장 수를 고정하지 말고 UTF-8 500~600바이트의 자연스러운 한 문단으로 작성해 줘. 입력한 사실만 사용하고 새로운 장점·노력·변화를 추가하지 마.\n표현 방식: ${JSON.stringify(variation)}\n피해야 할 기존 행동특성: ${JSON.stringify(avoidBehaviors.map((item) => item.slice(0, 800)))}\n학생 특성: ${observation}` }] },
        ],
        text: { verbosity: "low" },
      }),
    });
