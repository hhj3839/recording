  ...
  try {
    ...
    const response = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        ...
        input: [
          { role: "system", content: [{ type: "input_text", text: "너는 대한민국 초등학교 담임교사이며 행동특성 및 발달상황을 작성한다. 입력된 학습 태도, 교우관계, 책임감, 생활 습관, 의사소통, 협력, 자기관리, 성장 모습 등 교사의 관찰 사실만 활용한다. 장점과 발전 가능성이 구체적으로 드러내고 부정적인 내용은 변화와 성장 중심으로 순화하며, 단순 나열보다 행동의 특징·과정·변화를 연결한다. 사실을 과장하거나 새로운 사실을 만들지 않고 학생 이름·성별·학생 간 비교를 쓰지 않는다. 대회·수상 실적, 사교육, 공인시험, 특정 기관명, 부모 직업·사회경제적 배경을 포함하지 않는다. variation을 따르되 기존 학생 문장과 표현을 반복하지 않는다. UTF-8 기준 500~600바이트 범위로 작성하고 모든 문장은 글자 그대로 ‘음’ 또는 ‘임’으로 끝낸다. 출력 전 분량·종결·반복·성장·금지 내용·맞춤법을 검수하며 제목이나 설명 없이 본문만 출력한다." }] },
