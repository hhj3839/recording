  ...
  try {
    ...
    const response = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        ...
        input: [
          { role: "system", content: [{ type: "input_text", text: "너는 대한민국 초등학교 담임교사이며 학기말 학교생활기록부의 행동특성 및 종합의견을 작성하는 전문가이다. 입력된 학교생활 관찰 사실만 활용하여 학습 태도, 교우관계, 생활 습관, 수업 참여, 책임감, 협력성, 자기관리, 의사소통, 성장 모습이 종합적으로 이해되게 작성한다. 장점, 노력, 변화, 성장 가능성을 긍정적이고 균형 있게 드러내되 입력에 없는 사실은 만들지 않는다. 부정적인 내용은 사실을 바꾸지 않는 범위에서 성장 중심으로 순화하며 과장·단정·학생 간 비교를 피한다. 학생 이름과 성별을 쓰지 않는다. 성적·등수·수상 실적·대회·사교육·공인시험·특정 기관명·가정환경·부모 직업·사회경제적 배경·신체조건·학교폭력·징계·질병을 포함하지 않는다. variation을 따르되 기존 학생 문장과 표현을 반복하지 않는다. 정확히 4문장인 한 문단으로 작성하고 줄바꿈 없이 문장 사이를 한 칸으로 구분한다. UTF-8 510~540바이트를 목표로 하되 500~600바이트 범위 안에서 작성한다. 모든 문장은 글자 그대로 ‘음’ 또는 ‘임’으로 끝낸다. 출력 전 분량·종결·반복·입력 사실 일치·성장·금지 내용·맞춤법을 검수하며 제목이나 설명 없이 본문만 출력한다." }] },
          { role: "user", content: [{ type: "input_text", text: mode === "length" && currentBehavior
            ...
            : `다음 4~5가지 학생 특성을 바탕으로 표현과 문장 구조가 다른 행동특성을 작성해 줘. 정확히 4문장인 한 문단으로 작성하고 UTF-8 510~540바이트를 목표로 해 줘. 입력한 사실만 사용하고 새로운 장점·노력·변화를 추가하지 마.\n표현 방식: ${JSON.stringify(variation)}\n피해야 할 기존 행동특성: ${JSON.stringify(avoidBehaviors.map((item) => item.slice(0, 800)))}\n학생 특성: ${observation}` }] },
        ],
        text: { verbosity: "low" },
      }),
    });
    ...
    if (!validation.valid) return Response.json({
      error: "생성 결과가 500~600바이트·음/임 종결·성장·금지어 검수를 통과하지 못했습니다. 다시 생성해 주세요.",
      validation,
    }, { status: 502 });
