...
export async function GET() {
  try {
    ...
    const rows = await selectRows<Record<string, string | number>>("generated_comments", {
      owner_email: eq(user.email), class_id: eq(classId), order: "student_id.asc,subject.asc",
    });
...
export async function PUT(request: Request) {
  try {
    ...
    await upsertRows("generated_comments", [{
      student_id: studentId, subject, comment, updated_at: updatedAt, owner_email: user.email, class_id: classId,
