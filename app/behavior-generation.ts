  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      input: [
        ...
        { role: "user", content: [{ type: "input_text", text: `다음 학생 식별번호별 특성을 바탕으로 각각 행동특성을 작성해 줘. repairHint와 previousBehavior가 있으면 새로 작성하지 말고 previousBehavior를 기준으로 필요한 부분만 최소 수정한다. 500 또는 550바이트 경계에 맞추지 말고 515~535바이트를 목표로 한다. 부족하면 입력된 특성의 이미 언급된 행동을 더 구체적으로 연결하고, 길면 중복 수식어만 줄인다. 새로운 사실은 절대 추가하지 않는다.\n입력: ${JSON.stringify(inputs)}\n피해야 할 기존 시작 표현: ${JSON.stringify(avoidanceHints)}` }] },
      ],
      text: { verbosity: "low" },
    }),
  });
