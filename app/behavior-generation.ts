  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      input: [
        ...
        { role: "user", content: [{ type: "input_text", text: `다음 학생 식별번호별 특성을 바탕으로 각각 행동특성을 작성해 줘. repairHint와 previousBehavior가 있으면 새로 작성하지 말고 previousBehavior를 기준으로 repairHint에 적힌 음절 수만큼만 한 번 수정한다. 수정 후 스스로 UTF-8 바이트를 다시 계산한다. 부족하면 이미 언급된 행동의 과정만 구체화하고, 길면 중복 연결어와 수식어만 줄인다. 문장 수와 핵심 사실은 유지하며 새로운 사실은 절대 추가하지 않는다.\n입력: ${JSON.stringify(inputs)}\n피해야 할 기존 시작 표현: ${JSON.stringify(avoidanceHints)}` }] },
      ],
      text: { verbosity: "low" },
    }),
  });
