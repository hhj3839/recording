            ...
            <table className="comments-table subject-comments-table">
              ...
              <tbody>{roster.map((student, index) => {
                ...
                return <tr id={`comment-${student.id}`} key={student.id}>
                  ...
                  <td className="validation-cell issue-only-validation"><div>{!validation.endingsOk && <span className="fail">종결 확인</span>}{validation.forbidden.length > 0 && <span className="fail">금지어 확인</span>}{!validation.spellingOk && <span className="fail" title={validation.spellingIssues.join("\n")}>맞춤법 {validation.spellingIssues.length}건</span>}{similarStudents.length > 0 && <span className="fail">중복 {closest ? `${Math.round(closest.score * 100)}%` : "확인"}</span>}</div>{similarStudents.length > 0 && closest && <div className="similarity-detail"><strong>{closest.student.name} 학생과 {Math.round(closest.score * 100)}%</strong>{closest.overlaps.length > 0 && <span>겹치는 표현: {closest.overlaps.join(" · ")}</span>}</div>}{!validation.spellingOk && <ul className="spelling-issues">{validation.spellingIssues.map((issue, issueIndex) => <li key={issueIndex}>{issue}</li>)}</ul>}</td>
                </tr>;
              })}</tbody>
            </table>
          </div>
          ...
          <div className="comments-table-wrap">
            <table className="comments-table behavior-table behavior-split-table">
              ...
              <tbody>{roster.map((student) => {
                ...
                return <tr className={activeStudentId === student.id ? "active-reference-row" : ""} key={student.id}><td>{student.number ?? student.id}</td><td><strong>{student.name}</strong></td><td className="behavior-source-pane"><textarea className={sourceIssues.length ? "input-blocked" : ""} value={record.characteristic} onFocus={() => setActiveStudentId(student.id)} onChange={(event) => updateRecord(student.id, { characteristic: event.target.value })} onBlur={() => void saveRecord(student.id, records[student.id] ?? record)} placeholder={"관찰한 내용을 키워드·메모·문장 중 편한 방식으로 작성하세요.\n예: 질문을 자주 함 · 친구 말을 잘 들어줌 · 맡은 역할을 끝까지 함 · 발표에 자신감이 생김"} />{sourceIssues.length > 0 && <small className="source-warning">AI 전송 불가: {sourceIssues.join(" · ")}</small>}{record.characteristic.trim() && characteristicCount < 4 && <small className="source-warning characteristic-warning">특징을 {4 - characteristicCount}개 더 입력해 주세요.</small>}</td><td className="behavior-result-pane"><textarea value={record.behavior} onChange={(event) => updateRecord(student.id, { behavior: event.target.value })} onBlur={() => void saveRecord(student.id, records[student.id] ?? record)} placeholder={record.characteristic ? "행동특성을 생성하면 이곳에 결과가 표시됩니다." : "왼쪽에 관찰 키워드나 메모를 먼저 입력해 주세요."} />{record.behavior && <div className="behavior-result-meta"><div className="validation-cell behavior-validation issue-only-validation"><div>{!validation.lengthOk && <span className="fail">길이 확인</span>}{!validation.endingsOk && <span className="fail">음·임 종결 확인</span>}{!validation.growthIncluded && <span className="fail">성장 표현 확인</span>}{validation.forbidden.length > 0 && <span className="fail">금지어 확인</span>}{!validation.spellingOk && <span className="fail" title={validation.spellingIssues.join("\n")}>맞춤법 확인</span>}{validation.repeated.length > 0 && <span className="fail">반복 확인</span>}{similarStudents.length > 0 && <span className="fail">중복 {closest ? `${Math.round(closest.score * 100)}%` : "확인"}</span>}</div></div></div>}{similarStudents.length > 0 && closest && <div className="similarity-detail"><strong>{closest.student.name} 학생과 {Math.round(closest.score * 100)}%</strong>{closest.overlaps.length > 0 && <span>겹치는 표현: {closest.overlaps.join(" · ")}</span>}</div>}{!validation.spellingOk && <ul className="spelling-issues">{validation.spellingIssues.map((issue, issueIndex) => <li key={issueIndex}>{issue}</li>)}</ul>}<div className="comment-row-actions"><button disabled={!record.characteristic || !sourceValidation.valid || !!rewriteBusyKey} onMouseDown={(event) => event.preventDefault()} onClick={() => void rewriteBehavior(student.id, record, "regenerate")}>{rewriteBusyKey === `${student.id}|regenerate` ? "생성 중…" : "다시 생성"}</button>{record.behavior && !validation.lengthOk && <button disabled={!sourceValidation.valid || !!rewriteBusyKey} onMouseDown={(event) => event.preventDefault()} onClick={() => void rewriteBehavior(student.id, record, "length")}>{rewriteBusyKey === `${student.id}|length` ? "조정 중…" : "길이 조정"}</button>}</div></td></tr>;
              })}</tbody>
            </table>
          </div>
