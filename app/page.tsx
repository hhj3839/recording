...
function Behavior({ roster }: { roster: AssessmentStudent[] }) {
  type BehaviorViewMode = "input" | "review";
  ...
  const [viewMode, setViewMode] = useState<BehaviorViewMode>("input");
  ...
  const generatedCount = roster.filter((student) => records[student.id]?.behavior.trim()).length;
  const reviewIssueCount = roster.filter((student) => {
    const behavior = records[student.id]?.behavior?.trim() ?? "";
    return behavior && !validateRecord(behavior, true).valid;
  }).length;

  return (
    <section>
      <div className="page-heading">
        <div><p className="eyebrow">행동특성</p><h1>행동특성 작성</h1><p>{viewMode === "input" ? "학생에게서 관찰한 내용을 문장·메모·키워드 중 편한 방식으로 작성하세요." : "생성된 문장을 확인하고 필요한 부분만 수정하세요."}</p></div>
        <div className="ai-generate-actions">{formattedLastGeneratedAt && <span>마지막 생성 {formattedLastGeneratedAt}</span>}<button onClick={() => void generateAll()} disabled={loading || inputIssueCount > 0}>{loading ? generationProgress || "전체 생성 중…" : inputIssueCount ? `입력 확인 ${inputIssueCount}명` : "✦ 행동특성 생성"}</button>{viewMode === "review" && <button className="secondary" onClick={() => void copyBehaviors()} disabled={!generatedCount}>{copied ? "복사됨 ✓" : "결과 복사"}</button>}</div>
      </div>
      <div className="review-content behavior-table-content">
        <div className="behavior-mode-tabs" role="tablist" aria-label="행동특성 작성 단계"><button className={viewMode === "input" ? "active" : ""} onClick={() => setViewMode("input")} role="tab" aria-selected={viewMode === "input"}><span>1</span>관찰 내용 입력 <b>{eligibleStudentIds.length}/{roster.length}</b></button><button className={viewMode === "review" ? "active" : ""} onClick={() => setViewMode("review")} role="tab" aria-selected={viewMode === "review"}><span>2</span>결과 검토 <b>{generatedCount}/{roster.length}</b>{reviewIssueCount > 0 && <i>{reviewIssueCount}명 확인</i>}</button></div>
        <div className="workspace-toolbar behavior-table-toolbar"><div className="behavior-generation-summary">{viewMode === "input" ? <><span className="behavior-target-count"><small>생성 가능</small><strong>{eligibleStudentIds.length}/{roster.length}명</strong></span><span className="behavior-auto-rule"><i aria-hidden="true">✓</i>서로 다른 특징 4가지 이상</span></> : <><span className="behavior-target-count"><small>생성 완료</small><strong>{generatedCount}/{roster.length}명</strong></span><span className={reviewIssueCount ? "behavior-review-warning" : "behavior-auto-rule"}><i aria-hidden="true">{reviewIssueCount ? "!" : "✓"}</i>{reviewIssueCount ? `${reviewIssueCount}명 확인 필요` : "모든 결과 검수 완료"}</span></>}<span className="behavior-byte-target">권장 500~550B</span></div><div className="behavior-toolbar-actions">{viewMode === "input" && <button className="reference-open-button" onClick={() => setReferenceOpen((current) => !current)}>{referenceOpen ? "참고자료 닫기" : "참고 키워드"}</button>}<details className="behavior-more-actions"><summary>더보기</summary><button className="danger-text" onClick={() => void clearBehaviors()} disabled={loading || !generatedCount}>결과 초기화</button></details></div></div>
        {error && <p className="generation-error">! {error}</p>}
        {loading && <div className="comment-loading class-loading"><span>✦</span><p>입력된 모든 학생의 행동특성을 생성하고 있어요.</p></div>}
        <div className={`behavior-work-area ${viewMode === "input" && referenceOpen ? "with-reference" : ""}`}>
          <div className="comments-table-wrap">
            <table className={`comments-table behavior-table behavior-${viewMode}-table`}>
              <thead>{viewMode === "input" ? <tr><th>번호</th><th>이름</th><th>관찰 내용</th><th>입력 상태</th></tr> : <tr><th>번호</th><th>이름</th><th>행동특성</th><th>검수</th></tr>}</thead>
              <tbody>{roster.map((student) => {
                ...
                if (viewMode === "input") return <tr className={activeStudentId === student.id ? "active-reference-row" : ""} key={student.id}><td>{student.number ?? student.id}</td><td><strong>{student.name}</strong></td><td><textarea className={sourceIssues.length ? "input-blocked" : ""} value={record.characteristic} onFocus={() => setActiveStudentId(student.id)} onChange={(event) => updateRecord(student.id, { characteristic: event.target.value })} onBlur={() => void saveRecord(student.id, records[student.id] ?? record)} placeholder={"관찰한 내용을 자유롭게 작성하세요.\n예: 질문을 자주 함 · 친구의 말을 잘 들어줌 · 맡은 역할을 끝까지 수행함 · 발표에 자신감이 생김"} />{sourceIssues.length > 0 && <small className="source-warning">AI 전송 불가: {sourceIssues.join(" · ")}</small>}</td><td className="behavior-input-status"><strong className={sourceIssues.length ? "fail" : characteristicCount >= 4 ? "pass" : "pending"}>{sourceIssues.length ? "입력 확인" : characteristicCount >= 4 ? "생성 가능" : `${characteristicCount}/4개`}</strong><span>{sourceIssues.length ? "개인정보나 금지 내용을 삭제하세요." : characteristicCount >= 4 ? "생성 대상에 포함됩니다." : `특징을 ${4 - characteristicCount}개 더 입력하세요.`}</span></td></tr>;
                const comparisons = roster.filter((other) => other.id !== student.id).map((other) => ({
                  student: other,
                ...
                const similarStudents = comparisons.filter((item) => item.score >= 0.82);
                const closest = comparisons[0];
                const reviewPassed = validation.valid && !similarStudents.length;
                return <tr key={student.id}><td>{student.number ?? student.id}</td><td><strong>{student.name}</strong><small>{record.behavior ? `${validation.bytes}B` : "생성 전"}</small></td><td><textarea value={record.behavior} onChange={(event) => updateRecord(student.id, { behavior: event.target.value })} onBlur={() => void saveRecord(student.id, records[student.id] ?? record)} placeholder={record.characteristic ? "행동특성을 생성하면 결과가 표시됩니다." : "관찰 내용을 먼저 입력해 주세요."} /><div className="comment-row-actions"><button disabled={!record.characteristic || !sourceValidation.valid || !!rewriteBusyKey} onMouseDown={(event) => event.preventDefault()} onClick={() => void rewriteBehavior(student.id, record, "regenerate")}>{rewriteBusyKey === `${student.id}|regenerate` ? "생성 중…" : "다시 생성"}</button>{record.behavior && !validation.lengthOk && <button disabled={!sourceValidation.valid || !!rewriteBusyKey} onMouseDown={(event) => event.preventDefault()} onClick={() => void rewriteBehavior(student.id, record, "length")}>{rewriteBusyKey === `${student.id}|length` ? "조정 중…" : "길이 조정"}</button>}</div></td><td className="behavior-validation-summary"><strong className={record.behavior ? reviewPassed ? "pass" : "fail" : "pending"}>{record.behavior ? reviewPassed ? "검수 완료" : "확인 필요" : "생성 전"}</strong>{record.behavior && <span>{validation.bytes}B{!validation.lengthOk ? ` · 권장 범위 ${validation.bytes < 500 ? `${500 - validation.bytes}B 부족` : `${validation.bytes - 550}B 초과`}` : ""}</span>}<details><summary>검수 내용 보기</summary><div className="validation-cell behavior-validation"><div><span className={validation.lengthOk ? "pass" : "fail"}>500~550B</span><span className={validation.endingsOk ? "pass" : "fail"}>음·임 종결</span><span className={validation.growthIncluded ? "pass" : "fail"}>성장</span><span className={!validation.forbidden.length ? "pass" : "fail"}>금지어</span><span className={validation.spellingOk ? "pass" : "fail"}>맞춤법</span><span className={!validation.repeated.length ? "pass" : "fail"}>반복</span><span className={!similarStudents.length ? "pass" : "fail"}>중복 {closest?.score ? `${Math.round(closest.score * 100)}%` : "0%"}</span></div>{closest?.score > 0 && <div className="similarity-detail"><strong>{closest.student.name} 학생과 {Math.round(closest.score * 100)}%</strong>{closest.overlaps.length > 0 && <span>겹치는 표현: {closest.overlaps.join(" · ")}</span>}</div>}{!validation.spellingOk && <ul className="spelling-issues">{validation.spellingIssues.map((issue, issueIndex) => <li key={issueIndex}>{issue}</li>)}</ul>}</div></details></td></tr>;
              })}</tbody>
            </table>
          </div>
          {viewMode === "input" && referenceOpen && <aside className="behavior-reference-drawer">
