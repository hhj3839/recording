...
const unsupportedGroundingConcepts: Array<{
  ...
  {
    // 과목별 금지어가 아니라, 수행의 방식·정도에 해당하는 부사와 실제
    // 행동이 결합된 경우를 공통으로 검사한다. 대상 자체를 꾸미는
    // `알맞은 표정` 같은 관형 표현은 이 범주에 포함하지 않는다.
    label: "입력에 없는 수행 방식·정도",
    pattern: /(?:곧게|바르게|올바르게|정확(?:하게|히)|능숙(?:하게|히)|자연스럽게|효과적으로|체계적으로|적극적으로|주도적으로|꼼꼼하게|또렷하게|분명하게|자세히|깊이)\s*(?:그리|그림|긋|그음|계산|구별|분류|정리|기록|표현|설명|발표|작성|수행|활용|적용|해결|파악|이해|읽|쓰|말하|만들)/,
    evidencePattern: /(?:곧게|바르게|올바르게|정확(?:하게|히)|능숙(?:하게|히)|자연스럽게|효과적으로|체계적으로|적극적으로|주도적으로|꼼꼼하게|또렷하게|분명하게|자세히|깊이)\s*(?:그리|그림|긋|그음|계산|구별|분류|정리|기록|표현|설명|발표|작성|수행|활용|적용|해결|파악|이해|읽|쓰|말하|만들)/,
    blocking: true,
    warning: false,
  },
];
...
const semanticAtoms: SemanticAtom[] = [
  {
    label: "정확하고 알맞게 수행하기",
    criterion: /(?:정확(?:하게|히)|알맞게|올바르게|바르게)/,
    comment: /(?:정확(?:하게|히)|알맞게|올바르게|바르게)/,
  },
